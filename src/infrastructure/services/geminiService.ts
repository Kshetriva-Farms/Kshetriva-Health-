...
Use clear headings (###), bold text for key terms, bullet points for food items, and practical action items.`;
        ...
        console.warn('Gemini API call error, defaulting to local nutrition synthesis engine:', error);
      }
    }

    // Local fallback generator for specific question types
    let content = '';
...
*Log your progress in your Kshetriva Health+ dashboard daily to track improvements!*`;
  ...
  async generateIndianMealPlan(request: MealPlanRequest): Promise<IndianMealPlan> {
    const defaultPlan: IndianMealPlan = {
      id: `mp-${Date.now()}`,
      goal: request.goal,
      ...
      meals: {
        breakfast: {
          ...
          description: 'High-protein chickpea flour savory pancakes folded with chopped farm spinach and green spices.',
        ...
        lunch: {
          ...
          description: 'Comforting yellow moong dal tempered with cumin, garlic, and fresh spinach served with brown rice and carrot salad.',
...
Cuisine Style: ${request.cuisineStyle}`;
      ...
      console.warn('Gemini Meal Plan fallback to local generator.');
      return defaultPlan;
    }
  }
}
